...
| **Key** | Default · Pressed; live masters 56 x 72 px, HUG width, minimum 56 px; consumers FILL their rows | S-02 keypad and existing width/height proofs |
...
| **Language chip** | Live 119 x 56 px after EN minimum-width repair; EN 48 x 48 and বাংলা 61 x 48 segments; selected fill/text use `toggle/en/bg`, `toggle/en/fg`, `toggle/bn/bg`, `toggle/bn/fg` | shared headers; native Bangla preserved |
...
**Supporting-page reconciliation, 11 September 2026:** live inspection covered Cover, Flow/Sitemap/Research, Wireframes, Design System, Accessibility, Prototype guide and Redlines. After targeted updates, 876 visible text layers passed ancestor-containment checks; no overlapping top-level boards or searched stale auth/ride-count/old-language-size claims remained. The prototype guide, system reference and redlines were also rendered and visually reviewed. These are supporting-page checks, not a new all-screen or real-device audit. Original four starts and timers were read back unchanged; active runtime handlers were not edited.

**Current system reference:** [system-reference.png](system-reference.png), Figma node `274:1582` on page `5:5`, records live dimensions and the current collection inventory: 40 text styles; 61 token, 279 language, 18 prototype, 10 prototype-state and 18 auth-demo variables. Language variables include fonts, colours and selection state as well as copy. Tile `Value#26:31` now defaults to illustrative `80 Tk confirmed`; existing Home/Route instance overrides are retained. This replaces a stale `8 rides left` default, not the approved daily-fare policy.
...
**Key dimensions resolved by live read, 11 September:** both masters `26:27` / `26:29` measure 56 x 72 px, horizontal HUG and minimum width 56, agreeing with the [final-key-system recovery](../review/final-key-system.md). The earlier 72/48 report is superseded. The recovery report separately verifies 100 consumers restored to horizontal FILL and baseline dimensions, ten roots preserved, with zero measured containment failures. This supporting-page pass did not rerun all consumer or native-device tests. The current language master `26:108` is 119 x 56 px after the EN segment's 48 px minimum; the earlier 116 px width predates that change.
...
**Other choices**
- Touch targets: original tokens specify CTA 64 px, key/SOS 72 px; ticket SOS instances use 56 px. Responsive keys are not uniformly 72 px; §4 records the verified 56 x 72 masters and separate consumer recovery. Current language chip is 119 x 56 px with EN minimum 48 px, superseding 116/122 px-wide and 40 px-high snapshots. Coordinator found no interactive targets below 48 design px in the current structural scope; native dp/physical-device equivalence is not established.

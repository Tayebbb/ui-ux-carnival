...
**Current prototype qualifier, 11 September 2026:** the [22:38 Dhaka coordinator snapshot](review/prototype-repair-coordination.md) reports bounded browser PASS for the listed sample-auth, boarding, scanner, offline, route and SOS journeys, superseding older browser-blocked notes. This is supplied evidence, not testing by this documentation pass or field research. Four starts are Language/boarding `30:642`, Transit/SOS `29:646`, Full `27:229`, Offline `27:322`. Historical hold/overlay and ticket-timer proposals are SUPERSEDED for the demo: SOS uses five seconds and explicit Cancel, no hold; paid `29:551` uses a CTA with no root auto-timer. Daily 10 Tk does not convert 80 Tk into eight rides or approve grace, top-up or re-entry. DL-23's attributed OpenStreetMap uses illustrative positions, not GPS. Device/locale completeness, keyboard/TalkBack, anonymous sharing and real services remain gates; the decision chain below is preserved.
...
**Status: user-authorised design addition, 11 September 2026.** The coordinator now reports bounded sample sign-in/sign-out browser PASS; the problem statement below records the earlier entry state, not current routing. The [auth repair report](review/final-auth-repair.md) separately records 67 saved-action assertions and persisted Checking/result timers of 0.9/60 seconds. Service-error branches require seeded state and lack visible worker-chooser selectors. All six A-series destinations are top-level frames, not overlays/native auth. No server authentication, secure storage or native autofill is implemented; staff dashboards and boarding/payment/SOS policies are unchanged.
...
- Login assistance is not emergency dispatch. Target unauthenticated emergency-contact guidance without exposing worker/account information; use only client-verified contact details, currently unknown. Current S-07 Balance opens A-05 sign-out confirmation `204:1681`; Stay `204:1705` and header `204:1710` use `BACK`, avoiding the earlier Balance-confirmation history loop. Confirm clears auth demo state only, not payment/route/SOS state. Real session revocation and payment ownership remain production requirements; the SOS policy is unchanged.

### Focused Acceptance Checks

1. S-09 preserves English default and immediate Bangla selection and routes to **A-01 Sign in**, not directly to personal data. Only valid sample success reaches S-01; empty, rejected, offline, throttled and service-error states do not. A-02 help returns to A-01. A-03 fixed demo chooser, A-04 Checking, A-01e result and A-05 sign-out confirmation are top-level prototype frames, not overlays or worker services; S-10/S-11/S-12 IDs are not reassigned. The six auth low-fi sources are mapped in [03-structure/structure.md](03-structure/structure.md).
...
Verification [D]: earlier persisted Map/Stops, origin-return and offline-aware SOS checks, twelve 320/360/412 px English/Bangla proofs and 192 visible-text containment checks retain their snapshot scope. Recorded selected-tab contrast is 7.95:1; the independent finish review resolved its four listed visual fixes. The 22:38 coordinator subsequently reports ten browser Balance/Map/Stops return checks from Arrived, Full, Offline and 320/412 px Home proofs, plus scoped SOS cancellation. These are supplied findings, not new tests by this documentation pass, worker studies, native Android validation or a complete locale/device replay.
